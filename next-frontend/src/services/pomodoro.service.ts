...
import { IPomodoroSessionResponse, TypePomodoroSessionState, TypePomodoroRoundState } from "@/types/pomodoro.types";
...
class PomodoroService {
	...
	async updateSession(id: string, data: TypePomodoroSessionState) {
		const response = await axiosWithAuth.put<IPomodoroSessionResponse>(`${this.BASE_URL}/${id}`, data);
		return response;
	}
